...
class r_rule {
  ...
  public:
    ...
    inline int overlap(const r_rule &) const;  //duck Dec. 3
    inline void prune_mic_rule(const r_rule &, const addr_5tup &); // Mar 14
    inline friend int range_minus(vector<r_rule> &, const r_rule &); // Dec. 15 TODO to validate
...
/* member functions
 */
inline int r_rule::overlap (const r_rule & rr) const { // check whether another r_rule overlap with it.
    for (uint32_t i = 0; i < 4; ++i) {
    	//test
