...
class bucket_tree {
  ...
  public:
    bucket_tree();
    bucket_tree(rule_list &, uint32_t, bool test_bed = false, size_t = 0, int = -1, vector<bool> deleted_rules = vector<bool>());
    ~bucket_tree();
  ...
  public:
    //proactive
    void obtain_bucket_weight(const string & tracefile_str, int ruleId = -1, int splited_max = -1);
