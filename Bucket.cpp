...
pair<double, size_t> bucket::split(const vector<size_t> & dim , rule_list *rList) { //very important
    if (!sonList.empty())
        cleanson();
    //cout<<sonList.size()<<endl;
    uint32_t new_masks[4];
    size_t total_son_no = 1;

    for (size_t i = 0; i < 4; ++i) { // new mask
        new_masks[i] = addrs[i].mask;     //DEC.17这个addrs是从哪里赋值过来的？
        for (size_t j = 0; j < dim[i]; ++j) {
            if (~(new_masks[i]) == 0)
                return std::make_pair(-1, 0);

            new_masks[i] = (new_masks[i] >> 1) + (1 << 31); //DEC.18 二分
            //cout<<new_masks[i]<<endl;
            total_son_no *= 2;
        }
    }
   // cout<<total_son_no<<endl;
    ...
    for (size_t i = 0; i < total_son_no; ++i) {
        ...
        uint32_t id = i;
        for (size_t j = 0; j < 4; ++j) { // new pref  DEC.18 ????
        ...
        //cout<<son_ptr->related_rules.size()<<endl;
